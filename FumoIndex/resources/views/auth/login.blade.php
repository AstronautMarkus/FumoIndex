...
<div class="flex flex-col items-center justify-center mt-16 mb-6">
    <div class="bg-container backdrop-blur-sm rounded-3xl shadow-2xl border-4 border-secondary p-8 w-full max-w-md flex flex-col items-center">
        <h1 class="text-3xl md:text-4xl font-bold mb-6 text-primary text-center">Login</h1>

        <form method="POST" action="{{ route('auth.login') }}" class="w-full flex flex-col gap-4">
            @csrf
            <div>
        ...
        <div class="mt-6 text-center">
            ...
            <a href="{{ route('auth.register.form') }}" class="text-tertiary font-semibold hover:underline ml-2">Register</a>
        </div>
    </div>
</div>
