...
<div class="flex flex-col items-center justify-center mt-16 mb-6">
    <div class="bg-container backdrop-blur-sm rounded-3xl shadow-2xl border-4 border-secondary p-8 w-full max-w-6xl flex flex-col items-center">
        <h1 class="text-3xl md:text-4xl font-bold mb-6 text-primary text-center">Edit Fumo</h1>
        <form method="POST" action="{{ route('dashboard.fumos.update', $fumo->id) }}" enctype="multipart/form-data" class="w-full flex flex-col gap-4">
            @csrf
            @method('PUT')
            <div>
                <label for="fumo_name" class="block text-tertiary font-semibold mb-1">Fumo Name <span class="text-primary">*</span></label>
                <div class="relative">
                    <input id="fumo_name" type="text" name="fumo_name" value="{{ old('fumo_name', $fumo->fumo_name) }}" required class="input input-bordered w-full pl-10" placeholder="Fumo name">
            ...
            <div>
                ...
                <div class="relative">
                    <input id="gift_code" type="text" name="gift_code" value="{{ old('gift_code', $fumo->gift_code) }}" required class="input input-bordered w-full pl-10" placeholder="Gift code">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-6 w-full mt-4">
                    <div class="flex flex-col items-center">
                        ...
                        @if($fumo->fumo_image)
                            <img src="{{ $fumo->fumo_image }}" alt="{{ $fumo->fumo_name }}" class="w-36 object-cover pointer-events-none border-2 border-secondary" id="current-image">
                        @else
            ...
            <div>
                ...
                @error('gallery_images.*')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                @if($fumo->images && $fumo->images->count())
                    <div class="mt-4">
                        <p class="text-tertiary font-semibold mb-2">Current Gallery Images:</p>
                        <div class="flex flex-wrap gap-4">
                            @foreach($fumo->images as $img)
                                <div class="flex flex-col items-center">
                                    <img src="{{ $img->image_url }}" alt="Gallery image" class="w-24 h-24 object-cover border-2 border-secondary mb-2">
                                    <label class="inline-flex items-center text-sm">
                                        <input type="checkbox" name="delete_gallery[]" value="{{ $img->id }}" class="checkbox mr-2">
                                        Delete
                                    </label>
                                </div>
                            @endforeach
                        </div>
                        <small class="text-gray-500">Check images to delete from gallery on save.</small>
                    </div>
                @endif
            </div>
            <div>
                ...
                <div class="relative">
                    <input id="version" type="text" name="version" value="{{ old('version', $fumo->version) }}" required class="input input-bordered w-full pl-10" placeholder="Version">
            ...
            <div>
                ...
                <div class="relative">
                    <input id="official_url" type="url" name="official_url" value="{{ old('official_url', $fumo->official_url) }}" class="input input-bordered w-full pl-10" placeholder="https://...">
            ...
            <div>
                ...
                <div class="relative">
                    <textarea id="notes" name="notes" class="input input-bordered w-full pl-10" rows="2" placeholder="Notes...">{{ old('notes', $fumo->notes) }}</textarea>
            ...
            <button type="submit" class="btn btn-primary w-full mt-2 py-3 text-lg">Update Fumo</button>
        </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('franchise_id').addEventListener('change', function() {
        ...
        fetch(url)
            ...
            .then(characters => {
                ...
                // Get selected character ids from old input or model
                let selected = @json(old('character_ids', $fumo->character->pluck('id')->toArray()));
                characters.forEach(character => {
                    ...
                    if (selected.includes(character.id)) input.checked = true;
    ...
    // Auto-select franchise if editing
    const currentFranchise = @json(optional($fumo->character->first())->franchise->slug_name ?? null);
    if (currentFranchise) {
        const select = document.getElementById('franchise_id');
        // Wait for options to load
        setTimeout(() => {
            select.value = currentFranchise;
